...
class RecalcFilter(EMG):
    ...
    def __init__(self, emgfile, grid_names, to_filter= 0, rejected_chan = []):
    ...
    def convert_dict(self, muscle_names = ['TA']):
        try:          
            # Extract the samples and channel names from the Poly5Reader object
            self.samples = self.emgfile["RAW_SIGNAL"].to_numpy()
            self.sample_rate = self.emgfile["FSAMP"]
        ...
        nchans = self.emgfile['RAW_SIGNAL'].shape[1]
        ngrids = 1

        # read in the EMG trial data
        #emg_data = np.transpose(self.samples[1:-3, :])
        emg_data = np.transpose(self.samples)
        # create a dictionary containing all relevant signal parameters and data
        signal = dict(data = emg_data, fsamp = fsamp, nchans = nchans, ngrids = ngrids,grids = self.grid_names[:ngrids],muscles = muscle_names[:ngrids]) # discard the other muscle and grid entries, not relevant
        # if the signals were recorded with a feedback generated by ISpin, get the target and the path performed by the participant
        signal['target'] = self.emgfile['REF_SIGNAL'].to_numpy()
    ...
    def grid_formatter(self):
        ...
        for i in range(self.signal_dict['ngrids']):
            if grid_names[i] == '4-8-L':
                ...
                print(f"Grid name '{grid_names[i]}' not recognized.")
