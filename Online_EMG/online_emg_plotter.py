...
from Online_EMG.online_emg_chart import OnlineEMGChart

class OnlineEMGPlotter(Experiment_Plotter):
    """ForceFeedbackPlotter object
    """
    def __init__(self, meas_type , get_data_callback = None, name = "ForceForceFeedbackPlotter"):
        ...
        self.chart = OnlineEMGChart(self.chart)
    ...
    def update_chart(self, data_to_plot, time_span = None):
        ...
        for n_channel in range(len(data_to_plot)):
            ...
            if n_channel == 0 or n_channel == 1:
                offset = data_to_plot[1,int(np.shape(data_to_plot)[1]/2)]
                if self.scale <= 1e-3:
                    scale = 1
                else: 
                    scale = self.scale
                ...
                # For every motor unit channel a scale 1 is used with a offset of 0. 
                offset = 0
                scale = 50
                scales.append(scale)
                offsets.append(offset)
    ...
    def _filter_lists(self, list_to_filter):
        return [list_to_filter[i] for i in range(len(list_to_filter))]
    ...
    def get_channel_info(self, channels):
        ...
        for i in range(len(channels)):
            ...
            self._channel_units[i] = channel_unit
        self.update_enabled_channels()
        list_names = self._filter_lists(self._channel_names)
        list_units = self._filter_lists(self._channel_units)
        list_types = self._filter_lists(self._channel_types)
        return list_names, list_units, list_types
