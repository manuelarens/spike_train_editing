...
######################################## POST PROCESSING #####################################################
    ...
    def post_process_EMG_for_biofeedback(self,g,interval):
        
        self.mus_in_array = np.zeros(self.signal_dict['ngrids'])
        g += 1
        self.decomp_dict['MU_filters'] =  self.decomp_dict['dewhiten_mat'][interval] @ self.decomp_dict['MU_filters']
        # get the pulse train for the entire signal

        pulse_trains, discharge_times, _ = get_pulse_trains(self.signal_dict['data'], self.rejected_channels[0], np.squeeze(self.decomp_dict['MU_filters'], axis=0), [64], self.signal_dict['fsamp'],g-1)

        if np.shape(pulse_trains)[0] > 0: # if there are existing MUs
            
            self.mus_in_array[g-1] = 1 # if pulse trains were not extracted for this electrode, then it remains at a value of 0
           
            # removing duplicate MUs
            _, discharge_times_new, _ = remove_duplicates(self.decomp_dict['MU_filters'], pulse_trains, discharge_times, discharge_times, round(self.signal_dict['fsamp']/40), 0.00025, self.signal_dict['fsamp'], self.dup_thr)
            del pulse_trains, discharge_times

            # get the decomposition parameters for the biofeedback
            new_mu_filters = get_mu_filters(self.signal_dict['data'],self.rejected_channels[0], discharge_times_new, [64], g-1)
            # find the pulse trains again
            pulse_trains_new, discharge_times_new , _ = get_pulse_trains(self.signal_dict['data'], self.rejected_channels[0], new_mu_filters, [64], self.signal_dict['fsamp'],g-1)
            # get online parameters
            _ , inv_extended_data, norm, centroids =  get_online_parameters(self.signal_dict['data'], self.rejected_channels[0], new_mu_filters, [64], self.signal_dict['fsamp'],g-1)
            print('Shape of the found MU filters: '+ str(np.shape(new_mu_filters)))
            
            binary_spike_trains = get_binary_pulse_trains(discharge_times_new, np.size(self.signal_dict['data'][1]))
            # delete spectral leakage at beginning of the signal
            pulse_trains_new, discharge_times_new, binary_spike_trains = post_process_begin(pulse_trains_new, discharge_times_new, binary_spike_trains)

            self.decomp_dict['pulse_trains'][g-1] = pulse_trains_new
            self.decomp_dict['inv_extended_data'] = inv_extended_data
            self.decomp_dict['norm'] = norm
            self.decomp_dict['centroids'] = centroids
            self.mu_filters = new_mu_filters
            self.decomp_dict['BINARY_MUS_FIRING'] = binary_spike_trains
            self.decomp_dict['discharge_times'][g-1] = discharge_times_new
...
######################################## SAVING DATA #####################################################
    ...
    def save_EMG_decomposition_for_biofeedback(self, g,tracker, interval): # TODO: Check whether g or tracker? 
            print('Amount of motor units identified: ' + str(np.shape(self.decomp_dict['pulse_trains'][0])[0]))
            # adapted from OpenHDEMG, function save_to_json() 
            self.file_path_json = os.path.join(self.savefolder, self.filename +'_decomp.json')
            self.dict["SOURCE"] = "CUSTOMCSV"
            self.dict["FILENAME"] = "training40" 
            raw_emg = sort_raw_emg(self.signal_dict['data'],  self.signal_dict['grids'][0], self.signal_dict['fsamp'], self.emg_type)
            self.dict["RAW_SIGNAL"] = pd.DataFrame(raw_emg).T
            self.dict['DATA'] = pd.DataFrame(self.signal_dict['data']).T
            self.dict["REF_SIGNAL"] = pd.DataFrame(self.signal_dict['path'])
            self.dict["ACCURACY"] = pd.DataFrame(self.decomp_dict['SILs'])
            self.dict["IPTS"] = pd.DataFrame(self.decomp_dict['pulse_trains'][0]).T
            self.dict["MUPULSES"] = [np.array(item) for item in self.discharge_times]
            ...
            self.dict["BINARY_MUS_FIRING"] = pd.DataFrame(self.decomp_dict['BINARY_MUS_FIRING']).T
            self.dict["EMG_LENGTH"] = self.signal_dict['data'].shape[1]
            self.dict["NUMBER_OF_MUS"] = np.shape(self.decomp_dict['pulse_trains'][0])[0]
            self.dict['EMG_mask'] = pd.DataFrame(self.rejected_channels)
            self.dict["EXTRAS"] = pd.DataFrame(self.signal_dict['target'])
            ...
            # orient='split' is fundamental for performance.
            raw_signal = self.dict["RAW_SIGNAL"].to_json(orient='split')
            data = self.dict["DATA"].to_json(orient = 'split')
            ref_signal = self.dict["REF_SIGNAL"].to_json(orient='split')
            accuracy = self.dict["ACCURACY"].to_json(orient='split')
            ipts = self.dict["IPTS"].to_json(orient='split')
            centroids = self.dict["centroids"].to_json(orient='split')
            binary_mus_firing = self.dict["BINARY_MUS_FIRING"].to_json(orient='split')
            extras = self.dict["EXTRAS"].to_json(orient='split')
...
class online_EMG(EMG):
    ...
    # child class of EMG, so will inherit it's initialisaiton
    def __init__(self, filepath, rejected_channels = []):
        super().__init__()
        filename = filepath
        # filename =  r'C:/Users/natha/OneDrive - Universiteit Twente/Universiteit/Master/Internship/Python Interface/tmsi-python-interface/measurements/Training_measurement+ForceProfile-20240409_160538_decomp.json'
        self.emgfile = self.emg_from_json(filename)
        # self.data = np.transpose(self.emgfile["RAW_SIGNAL"].to_numpy())
        self.data = np.transpose(self.emgfile["DATA"].to_numpy())
        self.pulse_trains_saved = self.emgfile["IPTS"].T.to_numpy()
        self.discharge_times = self.emgfile["MUPULSES"]
        self.fsamp = int(self.emgfile["FSAMP"])
        self.EMG_mask = np.zeros((np.shape(self.data)[0],))
        self.rejected_chan = rejected_channels
        self.EMG_mask[self.rejected_chan] = 1
        self.accuracy = self.emgfile["ACCURACY"]
        self.nChan_per_elec =  [len(self.EMG_mask)]
    ...
    def emg_from_json(self, filepath):
        ...
        if source in ["DEMUSE", "OTB", "CUSTOMCSV", "DELSYS"]:
            ...
            data = pd.read_json(jsonemgfile["DATA"], orient='split')
            # Check dtypes for safety, little computational cost
            data.columns = raw_signal.columns.astype(int)
            data.index = raw_signal.index.astype(int)
            ...
            target = pd.read_json(jsonemgfile["EXTRAS"], orient='split')
    ...
    def preprocess_with_biofeedback(self):
        """
        Recalculating the separation filters based on discharge times 
        and saved training data. 
        """
        self.data_filt = self.data
        self.mu_filters = get_mu_filters(self.data_filt,self.EMG_mask, self.discharge_times, self.nChan_per_elec)
        print('Shape of recalculated MU_filters: ' + str(np.shape(self.mu_filters)))
        self.ext_factor, self.inv_extended_data, self.norm, self.centroids = get_online_parameters(self.data_filt, self.EMG_mask, self.mu_filters, self.nChan_per_elec, self.fsamp)
        self.mu_filters_recalc = np.transpose(self.mu_filters.T @ self.inv_extended_data)
        self.extend2 = np.zeros(((self.nChan_per_elec[0]-len(self.rejected_chan))*self.ext_factor, self.ext_factor-1))
        print('Preprocessing done')
        # self.plot()

    def callback(self, data):
        """
        Applies the separation filters to a block of data. 
        First, the channels with > 50 kOhm impedance are deleted. 
        Then the pulse trains and discharge times are extracted.
        """
        rejected_channels_slice = self.EMG_mask == 1
        data_filt = np.delete(data, rejected_channels_slice, 0) 
        self.pulse_trains, self.distimes_binary, _, self.extend2 = getspikesonline(data_filt, self.ext_factor, self.extend2, self.mu_filters_recalc, self.norm, self.centroids, self.fsamp)
        return self.distimes_binary
        
    def plot(self):
        
        # Plot the first subplot (force or force profile)
        plt.plot(self.pulse_trains_saved[0,:], color = 'red')  # Replace ... with your code to plot the force or force profile
        plt.plot(self.pulse_trains_calc[0,:], 'bo')
        plt.xlabel('Index')
        plt.ylabel('Value')
    ...
    def process(self, filename, grid_names = '4-8-L', muscle_names = 'TA'):
        """
        After a real-time decomposition measurement, the filters will be applied on the recorded EMG data to obtain the decomposition results. 
        This function loads the recorded data, applies the filters on the data, extracts the pulse trains and discharge times, post-processes the data by deleting any artefact at the beginning of the file and saves the the results.
        """
        self.select_file(filename)
        self.convert_poly5_xdf(grid_names = [grid_names], muscle_names =[muscle_names])
        #self.extend2 = np.zeros(((self.nChan_per_elec[0]-len(self.rejected_chan))*self.ext_factor, self.ext_factor-1))
        rejected_channels_slice = self.EMG_mask == 1
        full_data = np.delete(self.signal_dict['data'], rejected_channels_slice, 0) 
        
        self.pulse_trains, self.distimes_binary, self.distimes, self.extend2 = getspikesonline(full_data, self.ext_factor, self.extend2, self.mu_filters_recalc, self.norm, self.centroids, self.sample_rate)
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6))
        self.post_process()
        self.save_EMG_decomposition()
        
    def post_process(self):
        """ 
        Function that deletes the separation filter peak at the beginning of the pulse trains. 
        This is the result of applying the filters over the begin of the extended data where zeros are located.
        """

        self.distimes_binary[:,:50] = np.zeros((np.shape(self.distimes_binary)[0], 50))

        for i in range(len(self.distimes)):
            if len(self.distimes[i]) > 10:
                for j in range(len(self.distimes[i])-2):
                    if self.distimes[i][j] < 50: 
                        del self.distimes[i][j]           

        self.pulse_trains[:,:50] = np.zeros((np.shape(self.pulse_trains)[0], 50))

        
    def convert_poly5_xdf(self, grid_names = ['4-8-L'], muscle_names = ['BB']):
        """ 
        Function that extracts the raw data from a poly5 or xdf file. 
        """
        try:
            if self.filepath_poly5_xdf.lower().endswith('poly5'):
                data = Poly5Reader(self.filepath_poly5_xdf)
    ...
    def select_file(self, filepath):
        # filepath = filedialog.askopenfilename() # select a file and store path to the file 
        # filepath = r'C:/Users/natha/OneDrive - Universiteit Twente/Universiteit/Master/Internship/Python Interface/tmsi-python-interface/measurements/Training_measurement+ForceProfile-20240410_140900.poly5'
        self.filepath_poly5_xdf = filepath
        filepath = filepath.replace('/', '\\') # switch slashes
        self.filepath = filepath 
    ...
    def save_EMG_decomposition(self):
        #saves the Online decomposition results in a .json file.
        print(self.signal_dict['grids'])
        # adapted from OpenHDEMG, function save_to_json() 
        self.file_path_json = os.path.join(self.savefolder, self.filename +'_decomp.json')
        self.dict["SOURCE"] = "CUSTOMCSV"
        self.dict["FILENAME"] = "LIVE_DECOMP" 
        raw_emg = sort_raw_emg(self.signal_dict['data'],  self.signal_dict['grids'][0], self.signal_dict['fsamp'], self.emg_type)
        self.dict["RAW_SIGNAL"] = pd.DataFrame(raw_emg).T
        self.dict['DATA'] = pd.DataFrame(self.signal_dict['data']).T
        self.dict["REF_SIGNAL"] = pd.DataFrame(self.signal_dict['path'])
        self.dict["ACCURACY"] = pd.DataFrame(self.accuracy)
        self.dict["IPTS"] = pd.DataFrame(self.pulse_trains).T
        self.dict["MUPULSES"] = [np.array(item) for item in self.distimes]
        self.dict["FSAMP"] = float(self.signal_dict['fsamp'])
        self.dict["IED"] = float(self.ied)
        self.dict["BINARY_MUS_FIRING"] = pd.DataFrame(self.distimes_binary).T
        self.dict["EMG_LENGTH"] = self.signal_dict['data'].shape[1]
        self.dict["NUMBER_OF_MUS"] = np.shape(self.pulse_trains)[0]
        self.dict["EXTRAS"] = pd.DataFrame(self.signal_dict['target'])
