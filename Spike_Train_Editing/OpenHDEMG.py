...
class IptsPlotter:
    def __init__(
        ...
        self.mu_pulses = self._process_mu_pulses(emgfile["MUPULSES"])

        # Generate x-axis (seconds or samples)
        self.x_axis = (
            self.ipts.index / emgfile["FSAMP"]
            if timeinseconds
    ...
    def _process_mu_pulses(self, mupulses):
        if isinstance(mupulses, list):
            ...
            for pulses in mupulses:
                converted_pulses = [pulse / self.emgfile["FSAMP"] for pulse in pulses]
    ...
    def plot_current_mu(self):
        ...
        mu_index = self.current_index
        self.ax1.plot(self.x_axis, self.ipts[mu_index])
        self.ax1.set_ylabel(f"MU {mu_index + 1}")
        self.plot_peaks(mu_index)
        sil_value = compute_sil(self.ipts[mu_index], self.emgfile["MUPULSES"][mu_index])
    ...
    def plot_peaks(self, mu_index):
        pulses = self.mu_pulses[mu_index]
        for pulse in pulses:
            closest_idx = (np.abs(self.x_axis - pulse)).argmin()
            y_value = self.ipts.iloc[closest_idx, mu_index]
    ...
    def add_instructions(self):
        ...
        self.ax1.text(
            -0.16, 1.14, instructions, transform=self.ax1.transAxes, fontsize=10, verticalalignment="top"
        )
