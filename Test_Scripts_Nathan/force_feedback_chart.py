...
class ForceFeedbackChart(Chart):
    """Signal chart object
    """
    ...
    def setup_signals(self, n_signals, colors = None):
        ...
        for i in range(n_signals):
            ...
            #c.setPos(0,(i)*self._plot_offset) # Disabled to get all the lines on the same height
            self._curves.append(c)
        self._plotter_chart.window.setYRange(-1.5, 1.5, padding = 0)
    ...
    def update_chart(self, signals, time_span = None):
        ...
        if len(self._curves) != len(signals):
            self.setup_signals(len(signals))
        if len(signals) < 1:
            return
    ...
    def update_y_ticks(self, list_names, list_offsets, list_scales, list_units):
        ...
        else: # If the list of names is not empty, set ticks based on the values of last channel in the list. So most of the time values of the force profile. 
            tick_list_left = [[]]
            tick_list_left[0].append(
