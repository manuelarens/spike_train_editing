...
from ._experiment_plotter import Ui_Experiment_Plotter
from TMSiFrontend.chart import Chart
from TMSiFrontend.utilities.tmsi_style import TMSiStyle
...
class Experiment_Plotter(QtWidgets.QWidget, Ui_Experiment_Plotter):
    """Plotter interface
    """
    def __init__(self, meas_type, name = "Plotter"):
        """Initialize Plotter
    ...
    def set_configuration(self):
        if self.meas_type == 0:
            ...
            self.mvc_buttons.setVisible(False)
            self.training_buttons.setVisible(False)
            self.live.setVisible(False)
        elif self.meas_type == 1:
            self.group_amplitude.setVisible(False)
            self.calibration_box.setVisible(False)
            self.mvc_buttons.setVisible(True)
            self.training_buttons.setVisible(False)
            self.live.setVisible(False)
        elif self.meas_type == 2:
            self.group_amplitude.setVisible(False)
            self.calibration_box.setVisible(False)
            self.mvc_buttons.setVisible(False)
            self.training_buttons.setVisible(True)
            self.live.setVisible(False)
        else:
            self.group_amplitude.setVisible(False)
            self.calibration_box.setVisible(False)
