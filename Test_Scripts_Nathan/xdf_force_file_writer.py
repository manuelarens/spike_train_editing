...
class XdfWriter:
    ...
    def open(self, device, channel_names, channel_units, channel_types):
        """ Opens and initializes a xdf file-writer session.
            1. Opens the xdf-file
            2. Writes the magic code 'XDF:'
            3. Writes the FileHeader-chunk based on received channel names, units and types from the ForceFeedbackPlotterHelper as arguments
        ...
        self.device = device
        self.channel_names = channel_names # sets object channel_names
        self.channel_units = channel_units # sets object channel_units
        self.channel_types = channel_types # sets object channel_types
        self._sample_rate = self.device.get_device_sampling_frequency()
        self._num_channels = len(channel_names) # Sets object _num_channels
        ...
        try:
            ...
            self._write_stream_header_chunk(self._sample_rate)   
    ...
    def _write_stream_header_chunk(self, sample_rate, channels= None, imp_df=None):
        """ Writes the StreamHeader-chunk :
            - <info>-element with:
                ...
                - EEG meta-data, the channels-element ('<desc><channels>'):
                    - channel-name (<label>) Retrieved from ForceFeedbackPlotterHelper
                    - channel-type (<type>) Retrieved from ForceFeedbackPlotterHelper
                    - channel-unit-name (<unit>) Retrieved from ForceFeedbackPlotterHelper
        ...
        # Assemble <info>-root-element with the use of channel_names
        ...
        # Meta-data per channel, based on self.channel_names, self.channel_units, self.channel_types
        ...
        for j in range(num_channels):
            if i >= 0:
                ...
                # channel label
                item_label =  ET.SubElement(item_channel, 'label')
                item_label.text = self.channel_names[i]
                # channel content-type (EEG, EMG, EOG, ...)
                item_type = ET.SubElement(item_channel, 'type')
                
                if (self.channel_types[i] == ChannelType.UNI):
                    if not j==0:
                        item_type.text = 'EEG'
                        #channel location
                        # if self.add_ch_locs:
                        ...
                        item_type.text = 'CREF'
                elif (self.channel_types[i] == ChannelType.BIP):
                    item_type.text = 'BIP'
                elif (self.channel_types[i] == ChannelType.AUX):
                    item_type.text = 'AUX'
                elif (self.channel_types[i] == ChannelType.sensor):
                    item_type.text = 'sensor'
                elif (self.channel_types[i] == ChannelType.status):
                    item_type.text = 'status'
                elif (self.channel_types[i] == ChannelType.counter):
                ...
                # measurement unit (strongly preferred unit: microvolts)
                item_unit = ET.SubElement(item_channel, 'unit')
                item_unit.text = self.channel_units[i]
                i+=1
