...
/**
 * @file ${example_changing_channel_settings.py} 
...
try:
    ...
    if (len(discoveryList) > 0):
        ...
        print('\nActivated only BIP-channels, de-activated the other channels, the active channels are now :')
        for idx, ch in enumerate(dev.get_device_active_channels()):
             ch_name = ch.get_channel_name()
             ch_unit_name = ch.get_channel_unit_name()
             print('[{0}] : [{1}] in [{2}]'.format(idx, ch_name, ch_unit_name))
    
        # Rename some BIP-channels, print the updated active channel list
        #
        ...
        for idx, ch in enumerate(dev.get_device_channels()):
            if (ch.get_channel_name() == 'BIP 01'):
                 dev.set_device_channel_names(['V_EOG'], [idx])
            if (ch.get_channel_name() == 'BIP 02'):
                 dev.set_device_channel_names(['H_EOG'], [idx])

    
        print('\nRenamed channels [BIP 01] and [BIP 02], the names of the active channels are now :')
        for idx, ch in enumerate(dev.get_device_active_channels()):
             ch_name = ch.get_channel_name()
             ch_unit_name = ch.get_channel_unit_name()
             print('[{0}] : [{1}] in [{2}]'.format(idx, ch_name, ch_unit_name))
